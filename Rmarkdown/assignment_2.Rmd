...
yield_anom_2 <- ggplot()+
  geom_col(data = yield_anom_df, aes(x = year, y = y_anom, fill = "Historic Temp"), position = "dodge", width = 0.5, alpha = 0.8) +
  geom_col(data = yield_anom_df2, aes(x = year, y = y_anom_2c, fill = "2c Warmer"), position = "dodge", width = 0.5, alpha = 0.8) +
  theme_classic() +
   ggtitle("Yield Anom (1989 - 2010)\n") +
  xlab("\nYear") +
  ylab("Yield (tons/acre)") +
...
tn2_box <- ggplot(data = tn2_anom_merge, aes(y = y_anom, x = year, fill = scen)) +
  geom_boxplot(aes(fill = scen)) +
  geom_point(aes(color = scen), size = 3, alpha = 0.6) +
  theme_classic() +
  #ggtitle("Cachuma Water Storage and Historical Santa Cruz Inflow \nRainy Season Relationship (1999 - 2017)\n") +
  xlab("Year\n") +
  ylab("Yield") + 
...
#scale_y_continuous(expand = c(0,0), limits = c(-2,102)) +
 # scale_x_continuous(limits = c(1999,2020)) +
  #guides(color=guide_legend("Anom"))
